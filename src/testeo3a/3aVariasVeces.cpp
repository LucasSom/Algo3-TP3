...
int main() {
	...
	param.esquinaparam.first=-1;//-10
	param.esquinaparam.second=-1;//-10
	param.bordeparam.first=-1;//-3
	param.bordeparam.second=-1;//-3
	param.centroparam.first=-0.7;//8
	param.centroparam.second=-1;//8
	param.libertadparam.first=0.7;//1
	param.libertadparam.second=0.3;//1
	
	param.consecutivos.push_back(-0.427043);
	param.consecutivos.push_back(0.177773);
	param.consecutivos.push_back(-0.26863);
	
	param.consecparam.first=-0.8;
	param.consecparam.second=-1;
	
	
	param.extensiblesprox.push_back(-0.398409);
	param.extensiblesprox.push_back(-0.185101);
	param.extensiblesprox.push_back(0.881425); //esto es ganar para el, tiene que ser +inf
	
	param.extproxparam.first=-0.5;
	param.extproxparam.second=-1;
	
	
	param.extensibles.push_back(0.252257);
	param.extensibles.push_back(0.164281);
	param.extensibles.push_back(-0.394644); 
	
	param.extparam.first=0.6;
	param.extparam.second=-0.9;


	param.biextensibles.push_back(-0.777559);
	param.biextensibles.push_back(-0.100673);
	param.biextensibles.push_back(0.471494); //esto es ganar para el, es +inf
		
	param.biextparam.first=0.8;
	param.biextparam.second=0.2;
	//---------
