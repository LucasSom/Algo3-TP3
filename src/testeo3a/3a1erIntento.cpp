...
int main() {
	...
	param.esquinaparam.first=-0.0725883;//-10
	param.esquinaparam.second=0.038034;//-10
	param.bordeparam.first=-0.7;//-3
	param.bordeparam.second=-1;//-3
	param.centroparam.first=0.1;//8
	param.centroparam.second=-1;//8
	param.libertadparam.first=0.8;//1
	param.libertadparam.second=-1;//1
	
	param.consecutivos.push_back(0.229337);
	param.consecutivos.push_back(0.0689698);
	param.consecutivos.push_back(0.668262);
	
	param.consecparam.first=-0.0735537;
	param.consecparam.second=0.777079;
	
	
	param.extensiblesprox.push_back(0.786683);
	param.extensiblesprox.push_back(0.730204);
	param.extensiblesprox.push_back(-0.907692); //esto es ganar para el, tiene que ser +inf
	
	param.extproxparam.first=0.7;
	param.extproxparam.second=-1;
	
	
	param.extensibles.push_back(-0.727517);
	param.extensibles.push_back(-0.658663);
	param.extensibles.push_back(-0.420025); 
	
	param.extparam.first=0.7;
	param.extparam.second=-0.2;


	param.biextensibles.push_back(0.856624);
	param.biextensibles.push_back(0.819945);
	param.biextensibles.push_back(0.464382); //esto es ganar para el, es +inf
		
	param.biextparam.first=-0.0999999;
