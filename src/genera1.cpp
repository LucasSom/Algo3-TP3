...
pair<int,int> minimax2(int rows, int columns, int c, int p, vector<vector<int>> tablero,  bool maximizo, int alfa, int beta, int ultimajugada){
	...
	for(int i=0; i<posibles.size(); ++i){
		...
		if (maximizo) {
			posibles[i].second = minimax2(rows, columns, c, p-1, tablero2, not maximizo, alfa, beta, posibles[i].first).second;
...
int main() {
    ...
    while (true) {
		...
		ofstream myfile;
		myfile.open ("1sinpoda.csv", std::ios_base::app);
		ofstream myfile2;
		myfile2.open ("1poda.csv", std::ios_base::app);
        ...
        if (go_first == "vos") {
			...
			if(tablero[h].size()<rows){ 
				posibles.push_back(h);
				movposibles=movposibles+rows-tablero[h].size();
				}
			//el -88888888 es que aun no se el puntaje que da ese nodo en el arbol minimax
			}
			movposibles=min(p/2,movposibles);
			...
			std::clock_t a = clock();
            move = minimax2(rows, columns, c, p, tablero, true,-2,2, -1).first;
            clock_t b = clock() - a;
			double tiempo2 = b / (double) CLOCKS_PER_SEC;
            
			myfile << pow(posibles.size(),movposibles) << "," << tiempo << "\n";
			myfile2 << pow(posibles.size(),movposibles) << "," << tiempo2 << "\n";
        ...
        while (true) {
			...
			std::clock_t a = clock();
            move = minimax2(rows, columns, c, p, tablero, true,-2,2, std::stoi(msg)).first;
            clock_t b = clock() - a;
			double tiempo2 = b / (double) CLOCKS_PER_SEC;
