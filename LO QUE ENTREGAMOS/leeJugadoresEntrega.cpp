...
parametro leerParametroDeValores(int c){
	...
	#ifdef BUENO 
	valores.open ("JugadorPropuestoPorNosotros.txt");
	#endif
	
	#ifdef GRID 
	valores.open ("jugador3aBackInteligenteSinVentaja.txt");
	#endif

	#ifdef GENFITNESS2
	valores.open ("MejorResultadoGenetico4EnLinea.txt");
	#endif

	#ifdef GENBACKTRACK
	valores.open ("jugadorGeneticoBacktrack.txt");
	#endif
