...
\begin{algorithm}
   \caption{main}
    \begin{algorithmic}[1]
    \Function{main}{}
        \State \texttt{leer} parámetros: \textit{color, oponent\_color, columns, rows, c, p}
        \State inicializar $tablero$
		\State \texttt{leer} quién empieza
		\If{empiezo yo}
			\State $movida$ = \texttt{minimax}($rows$, $columns$, $c$, $p$, $tablero$, \textbf{true}, -1).first
			\State \texttt{AgregarAtras}($tablero[movida]$, 1)
			\State resto 1 a $p$
			\State \texttt{jugar}($movida$)		
		\EndIf
        \State \textbf{while} $true$ \textbf{do}

            	\State \texttt{leer} $mensaje$
            	\If{el $mensaje$ es de finalización}
            		\State \textbf{break}
            	\EndIf
    			\State \texttt{AgregarAtras}($tablero$[movida del otro jugador], 2)        	
            	\State resto 1 a $p$
            	\State $movida$ = \texttt{minimax}($rows$, $columns$, $c$, $p$, $tablero$, \textbf{true}, -1).first
            	\State \texttt{AgregarAtras}($tablero[movida]$, 1)
            	\State resto 1 a $p$
...
\begin{algorithm}
   \caption{minimax}
    ...
    \Function{minimax}{\textbf{int} rows, \textbf{int} columns, \textbf{int} c, \textbf{int} p, \textit{matriz}(\textbf{int} \textbf{int}) tablero,  \textbf{bool} maximizo, \textbf{int} ultimajugada}
		...
		\If{Maximizo}
		    \State \textbf{return} la columna que da el máximo y su resultado
		\Else
		    \State \textbf{return} la columna que da el mínimo y su resultado
		\EndIf
...
\subsubsection{El Pseudocódigo}
En el main a la variable $movida$ se le asigna el resultado de la función \texttt{parametrizable} que es el algoritmo goloso propuesto para esta parte del problema. Esta toma los mismos parámetros que \texttt{minimax} con el agregado del \textbf{parametro} $param$ y devuelve el número de columna que se debe jugar.
...
La variable $tablero$ también se pasa por referencia pero como \texttt{const}.
\begin{algorithm}
\caption{puntaje}
    \begin{algorithmic}[1]
    \Function{puntaje}{\textbf{int} rows, \textbf{int} columns, \textbf{int} c, \textbf{int} p, matriz(\textbf{int}, \textbf{int}) tablero, \textbf{int} ultimajugada, \textbf{parametro} param}
		...
		\EndIf    	
    	\State cuento los que estan en el borde de cada jugador
    	\State cuento la cantidad de fichas que estan en una esquina de cada jugador
    	\State cuento la cantidad de fichas en el centro de cada jugador
		\State cuento la cantidad de casillas libres alrededor de cada ficha de cada jugador
		\State cuento la cantidad de fichas consecutivas de cada jugador
		\State cuento cantidad de inmediatemente extensibles en la próxima jugada de cada jugador
		\State cuento cantidad de  extensibles en cualquier momento de cada jugador
		\State cuento cantidad de  bi-extensibles (extensibles en ambos sentidos) en cualquier momento de cada jugador
...
\begin{algortithmic}[1]
	...
	\For{$generacion$ = [1..$totalgeneraciones$)}
		\State poblacion = \texttt{seleccionNatural}
		\For{$i \in poblacion$}
