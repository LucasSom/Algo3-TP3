...
\textbf{SOY SOMA. MSG PARA SOMA: MI REVISION LLEGO HASTA ACA}
...
La función \texttt{optimizar} toma un parámetro $pp$ de un \textbf{parametro} $param$ y busca el valor de ese parámetro para el que se maximiza la performance de $param$. Es decir, se juegan varios partidos con diferentes valores de $pp$ y devuelve el valor para el cual se ganaron más partidas.

\begin{algorithm}
\SetKwInOut{Input}{Input}


\Input{\textbf{int} rows, \textbf{int} columns, \textbf{int} c, \textbf{int} p, \textbf{parametro} param}

\textbf{parametro} $mejor$

\For{$pp \leftarrow $ parámetros de $param$}
	{

	$mejor$ = \texttt{optimizar} parámetro $pp$
